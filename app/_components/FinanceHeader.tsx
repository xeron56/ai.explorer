...
function NavLink({
  ...
  if (compact) {
    return (
      <Link
        href={item.href}
        className={`whitespace-nowrap ${active ? "text-[#13924a]" : "text-[#546476]"}`}
      >
        {item.label}
      </Link>
    );
  ...
  return (
    <Link
      ...
      className={`relative text-[13px] font-semibold transition-colors ${
        active ? "text-[#13924a]" : "text-[#425466] hover:text-[#0f1728]"
      }`}
    >
      {item.label}
      {active && <span className="absolute -bottom-[26px] left-0 h-[2px] w-full rounded-full bg-[#22a35f]" />}
    </Link>
  );
}
