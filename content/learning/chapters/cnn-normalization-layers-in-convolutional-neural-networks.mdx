...
<NormalizationTypesExplorer />

*Interactive Normalization Explorer: Select BatchNorm, LayerNorm, InstanceNorm, or GroupNorm. Hover any sample/channel block to see which tensor slices $[N, C, H, W]$ are standardized together.*
...
<NormalizationEffectVisualizer />

*Interactive Scale & Shift Simulator: Adjust learnable scale ($\gamma$) and shift ($\beta$) parameters to see how normalization standardizes raw activations to zero mean ($\mu=0$) and unit variance ($\sigma=1$), then reconstructs optimal output distributions $y = \gamma \hat{x} + \beta$.*
