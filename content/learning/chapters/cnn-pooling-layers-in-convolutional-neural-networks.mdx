...
<TranslationInvarianceVisualizer />

*Interactive Translation Invariance Simulator: Try shifting the input feature pattern horizontally ($\Delta x$) or vertically ($\Delta y$). Notice how Max Pooling retains the peak feature response in the pooled output feature map.*
...
<PoolingOperationVisualizer />

*Interactive Pooling Visualizer: Select Max, Average, Global, or Mixed Pooling. Hover any output cell to trace the input window region and see the live arithmetic reduction step.*
