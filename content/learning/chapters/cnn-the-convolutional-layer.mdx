...
*Interactive Visualizer: Try hovering over the input matrix, kernel, or output cells to trace exact weight multiplications. Change presets or click cells to test custom feature extraction filters.*
...
*Interactive Dimension Calculator: Test different input dimensions, kernel sizes, stride ($s$), padding ($p$), and dilation ($d$). Toggle Transposed mode to see upsampling calculations.*
...
*Interactive Convolution Explorer: Compare parameters, FLOPs, and receptive fields for Standard, Depthwise Separable, Dilated, and Transposed convolutions.*
